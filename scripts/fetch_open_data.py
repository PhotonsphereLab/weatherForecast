...
PL_PARAMS  = ["u", "v", "gh"]
PL_LEVELS  = [850, 500]
STEPS_H    = [0, 6]  # extend later (12, 24)

# Use ECMWF Open Data defaults (IFS 0.25°). Don't force stream/endpoint.
c = Client(source="ecmwf")

def fetch_sfc(param: str):
    tmp = OUTDIR / f"_tmp_{param}.grib2"
    # The client handles the latest available run. No stream/date/time needed.
    c.retrieve(
        type="fc",
        step=STEPS_H,        # [0, 6]
        param=param,
    ...
    final = OUTDIR / f"{param}_sfc_latest.grib2"   # stable name; matches msl_sfc_*.grib2 pattern
