...
PL_PARAMS  = ["u","v","gh"]
PL_LEVELS  = [850, 500]
STEPS_H    = [0, 6]  # add 12/24 later

now = dt.datetime.utcnow()
cycle = (now.hour // 6) * 6
run = now.replace(hour=cycle, minute=0, second=0, microsecond=0)

c = Client(source="ecmwf", endpoint="https://data.ecmwf.int/forecasts")

def fetch_sfc(param: str):
    tmp = OUTDIR / f"_tmp_{param}_{run:%Y%m%d%H}.grib2"
    if not tmp.exists():
        c.retrieve(stream="hres", time=f"{run:%H}", date=f"{run:%Y-%m-%d}",
                   type="fc", step="/".join(str(s) for s in STEPS_H),
                   param=param, target=str(tmp))
...
def fetch_pl(param: str, level: int):
    ...
    if not tmp.exists():
        c.retrieve(stream="hres", time=f"{run:%H}", date=f"{run:%Y-%m-%d}",
                   type="fc", step="/".join(str(s) for s in STEPS_H),
                   param=param, levtype="pl", levelist=str(level), target=str(tmp))
    final = OUTDIR / f"{param}_pl_{level}_{run:%Y%m%d%H}.grib2"
    tmp.replace(final)
    print("[OK] PL", param, level, "->", final.name)

def main():
    for p in SFC_PARAMS: fetch_sfc(p)
    for p in PL_PARAMS:
        for lv in PL_LEVELS: fetch_pl(p, lv)
